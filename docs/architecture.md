...
После загрузки RC API сохраняет `DocumentExtraction` со статусом `PENDING` и добавляет job в очередь BullMQ. Отдельный worker будет читать документ из object storage, вызывать AI provider и сохранять structured result с confidence для каждого поля.
