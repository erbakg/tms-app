...
Доступен `ADMIN` и `DISPATCHER`. Возвращает упорядоченный по имени список водителей для назначения, содержащий только `id`, `fullName`, `email` и `role`. Hash пароля и любые секреты в directory response отсутствуют. Сейчас поддерживается только фильтр `role=DRIVER` — это не общий endpoint экспорта пользователей.
